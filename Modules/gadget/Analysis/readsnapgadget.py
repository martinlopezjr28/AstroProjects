...
def readsnap(sdir,snum,ptype,
    ...
    pot=np.zeros([npartTotal[ptype]],dtype=np.float64)
    if (ptype==0):
        ugas=np.copy(mass)
        rho=np.copy(mass)
        hsml=np.copy(mass)
    ...
    for i_file in range(numfiles):
        ...
        if(npart[ptype]>0):
            ...
            pot[nL:nR]=input_struct[bname + "Potential"]
            ...
            if (ptype==5) and (skip_bh==0):
                bhmass[nL:nR]=input_struct[bname+"Masses"]
                bhmdot[nL:nR]=[0.0]
                ##This code I think is for when you turn on black holes but I didn't so yeah
    ...
    file.close();
    if (ptype==0):
        return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'u':ugas,'rho':rho,'h':hsml,
                'ne':nume,'nh':numh,'sfr':sfr,'z':metal, 'pot':pot};
    if (ptype==4):
        return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'z':metal,'age':stellage}
    if (ptype==5) and (skip_bh==0):
        return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids,'mbh':bhmass,'mdot':bhmdot}
    return {'k':1,'p':pos,'v':vel,'m':mass,'id':ids, 'pot':pot}
