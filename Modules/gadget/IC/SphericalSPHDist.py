...
def getSPHParticles(r_low,P_mass,M_int,rho_int,u_int,Rstar,rotshell,gaussRad,Nsigma,dr_sigma,debug=False, writeout_file = None):
    ...
    writeout_file.write(
        '\n'+
        '=============================================\n'+
        'Created N = ' + str(count) + ' shells\n'+
        'with a total of Np = ' + str(len(xpos)) +' particles\n'+
        '=============================================\n'
        )
...
def get_particle_properties(mp,pos,prel,vrel,vrad,vrot,rho_int,u_int):
    '''
    Get the SPH particle properties (rho, mp, u, h) from their position

    Receives:
    * (xpos, ypos, zpos)  -> Three 1D Arrays with all SPH particles' positions (x,y,z)
    * mp -> Array with particle's masses
    ...
    # Transform to cartesian and assign all properties to particles
    ...
    vx_f = np.zeros(nsph) + np.ones(nsph)*vrel[0]
    vy_f = np.zeros(nsph) + np.ones(nsph)*vrel[1]
    vz_f = np.zeros(nsph) + np.ones(nsph)*vrel[2]

    u_f = u_int(r_f)
    m_f = mp
    # m_f = (Mstar/nsph)*np.ones(nsph)
    ...
    h_f = eta*(m_f/rho_f)**(1./3.)
    id_f = np.arange(1,nsph+1)

    ptype = [0]*len(h_f) #int(np.zeros(len(h_f)))
...
def remove_gasparticles(xpos,ypos,zpos,mp,R_core):
    ...
    * M_core -> mass inside the boundary shell.
    ...
    print '--------------------------------------'
    print 'NSPH_initial  |  Ncore + Nenv |  Nbdry '
    print Ni,'          ',N_core + N_env,'         ',N_bdry
    print ''
    ...
    print '--------------------------------------'
    print 'Mass_initial  |  Mcore + Menv |  Mcore (point mass) '
    print round(sum(mp)/Mo,4),'          ',round((Mc + Menv)/Mo,4),'         ',round(Mc/Mo,4)
    print ''
...
def make_hdf5_from_table(data,filename_out,
                      ...
                      MassUnit=c.msun,writeout_file = None):
    """
    ...
    writeout_file.write("\nParticle numbers [N0,N1,N2,N3,N4,N5]: " + str(npart) + '\n')
